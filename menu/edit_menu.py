...
from models.storage import Storage, connection_db
        ...
        if conf == 'y':
            ...
            try:
                with connection.cursor() as cursor:
                    cursor.execute(f'DELETE FROM questions WHERE id={selected_id};')
...
class EditQuestion(BaseMenu):
    ...
    def new_text(self):
        ...
        try:
            with connection.cursor() as cursor:
                for question in storage.questions:        
                    quer = f'UPDATE questions SET text=\'{self.question.text}\'  WHERE id={self.question.id};'
    ...
    def new_answers(self):
        ...
        try:
            with connection.cursor() as cursor:
                for question in storage.questions:        
                    quer = f'UPDATE questions SET answers=\'{json.dumps(self.question.answers)}\'  WHERE id={self.question.id};'
                print(quer)
                cursor.execute(quer)
    ...
    def reset_statistics(self):
        ...
        try:
            with connection.cursor() as cursor:
                for question in storage.questions:
                    quer = f'UPDATE questions SET stat_wrg=0, stat_cor=0 WHERE id={self.question.id};'
